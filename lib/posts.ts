...
import { formatFrenchDate, renderMarkdown, toStringArray } from "@/lib/markdown";
...
function readPost(fileName: string, glossary: GlossaryLink[]): Post {
  ...
  return {
    ...
    html: linkGlossaryTerms(renderMarkdown(content), glossary),
  };
}
...
export function getAllPosts(): Post[] {
  ...
  // Construite une fois par lecture, pas une fois par article : sans cela, le
  // dossier du glossaire serait relu treize fois à chaque appel.
  const glossary = buildGlossaryLinks(getAllGlossaryEntries());
  const posts = files.map((fileName) => readPost(fileName, glossary));
