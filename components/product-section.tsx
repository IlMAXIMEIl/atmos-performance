      ...
      */}
      <div ref={railRef} className="mt-16 lg:h-[300vh]">
        <div className="lg:sticky lg:top-24 lg:flex lg:h-[calc(100vh-6rem)] lg:items-center">
          ...
          */}
              <div className="mt-10 flex flex-col border-t border-white/[0.07] lg:relative lg:mt-9 lg:block lg:min-h-[13rem] lg:border-t-0">
                {SPECS.map((spec) => (
                  <div
                    key={spec.id}
              ...
              {/*
                La fiche technique monte dans la colonne épinglée, sous le
                chapitre qui change.

                Deux problèmes réglés d'un coup. Elle donne à la colonne de
                droite la hauteur de la carte : un chapitre seul, trois lignes
                de texte, laissait un demi-écran de vide dans une scène centrée
                verticalement. Et elle cesse d'être reléguée en bas de section,
                loin du produit qu'elle décrit — elle reste sous les yeux
                pendant toute la traversée.

                Sous `lg`, c'est le dépliant en fin de section qui prend le
                relais. Les deux rendus partagent la table `DATASHEET`.
              */}
              <dl className="mt-10 hidden border-t border-white/[0.07] pt-7 lg:block">
                <div className="text-[0.62rem] font-medium tracking-[0.24em] text-white/40 uppercase">
                  Fiche technique
                </div>
                ...
                <div className="mt-4">
                  {DATASHEET.map((row) => (
                    <div
                      key={row.label}
                      className="flex items-baseline justify-between gap-6 border-b border-white/[0.05] py-2 last:border-b-0"
                    >
                      <dt className="shrink-0 text-[0.66rem] font-light tracking-[0.14em] text-white/35 uppercase">
                        {row.label}
                      </dt>
                      <dd className="text-right text-[0.82rem] font-light text-white/70 text-pretty">
                        {row.value}
                      </dd>
                    </div>
                  ))}
